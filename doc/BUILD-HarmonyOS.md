...
| 4.6.0 | `versions/4.6.0/patches/` | 16（新增 seccomp zlib 变通方案） |
...
**修复方案**:
1. `src/library/tools/R/install.R`: 强制所有 sysdata 压缩为 `FALSE`；`data_compress <- FALSE`（跳过包数据压缩）；`makeLazyLoading(compress=FALSE)`
2. `src/library/tools/R/makeLazyLoad.R`: `sysdata2LazyLoadDB()` 中检测 gzip magic bytes，通过外部 `gzip -dc pipe` 解压 `.rda`；mapfile 的 `saveRDS` 取消压缩
3. `src/library/tools/R/build.R`: `build_partial_Rd_db_path` 的 `saveRDS` 取消压缩
4. `src/library/tools/R/Rd.R`: `readRDS(built_file)` 添加三层 fallback：先尝试原生 readRDS → memDecompress(gzip) → 外部 `gzip -dc pipe`
5. `src/library/tools/R/admin.R`: `readRDS(indexname)` 添加 memDecompress fallback
6. `share/make/lazycomp.mk` 和 `share/make/basepkg.mk`: 编译时 `compress=FALSE`
7. `apply-patches.sh`: 内联 python3 脚本修改 `makebasedb.R`（base 包）、`methods/R/zzz.R`（methods 包）中的 `makeLazyLoadDB` 调用
